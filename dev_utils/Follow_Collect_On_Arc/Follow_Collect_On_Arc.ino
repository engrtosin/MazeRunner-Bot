...
//
//  Arc-pause feature : while executing the WL_ARC outer-corner
//                      recovery, if a ball is detected AND its
//                      Y pixel is >= BALL_ARC_COLLECT_Y_THRESHOLD
//                      (ball is close enough to collect), the arc
//                      is paused, the robot runs the full
//                      TRACK → COLLECT → COLLECTED ball cycle,
//                      then resumes the arc exactly where it left
//                      off (timeout clock is preserved).
//
...
// ================= BALL Y-THRESHOLD FOR ARC COLLECTION =================
// When the ball's Y pixel (0 = top of frame, 254 = bottom) is >= this value
// the ball is considered close enough to collect during an arc manoeuvre.
// Increase to collect only when the ball is very close (large in frame);
// decrease to trigger collection earlier.
#define BALL_ARC_COLLECT_Y_THRESHOLD  160
...
// ── Arc-pause: track how long the arc was already running before it was
//    interrupted so the timeout clock is seamlessly resumed afterwards. ──
bool          arcInterruptedByBall = false;
unsigned long arcElapsedBeforePause = 0;   // ms already spent in arc
  ...
  switch (wallLostPhase) {
    ...
    case WL_CREEP: {
      ...
      if (creepL >= CREEP_COUNTS_L_WF && creepR >= CREEP_COUNTS_R_WF) {
        hardStop();
        arcStartMs    = millis();
        arcElapsedBeforePause   = 0;      // fresh arc — no prior elapsed time
    ...
    case WL_ARC: {
      ...
      // ── Arc timeout (using elapsed time to survive pauses) ──────
      unsigned long arcElapsedNow = arcElapsedBeforePause + (millis() - arcStartMs);
      ...
      // ── Ball detected during arc — pause and collect ─────────────
      // Trigger only when the ball is close enough (Y >= threshold)
      // so the robot doesn't abort every arc for a distant ball.
      if (ballVisible() && pkt[1] >= BALL_ARC_COLLECT_Y_THRESHOLD) {
        hardStop();
        // Freeze how much arc time has been consumed so far.
        ...
        ballPhaseMs = millis();
        robotState  = STATE_BALL_OVERRIDE;
        // Serial.println("WL | arc paused for ball -> BALL_OVERRIDE");
        return;
      }
...
void doBallOverride() {
  ...
  switch (ballPhase) {
    ...
    // ── Collected: pause then return to caller ─────────────────────
    case BP_COLLECTED: {
      hardStop();
      rollersOff();
      if (millis() - ballPhaseMs > 2000) {
        ...
        if (arcInterruptedByBall) {
          // ── Resume the arc that was interrupted ──────────────────
          // Restart the arc segment timer so elapsed time accounting
          // works correctly (arcElapsedBeforePause already holds the
          // time spent before the pause).
          arcStartMs   = millis();
          wallLostPhase = WL_ARC;
          stateEntryMs  = millis();
          robotState    = STATE_WALL_LOST;
          // Serial.println("Ball | done -> resume WL_ARC");
        } else {
          // ── Normal ball collection from wall-follow ───────────────
          stateEntryMs = millis();
          robotState   = STATE_WALL_FOLLOW;
...
void setup() {
  ...
  Serial.print("Arc collect Y    : "); Serial.print(BALL_ARC_COLLECT_Y_THRESHOLD);  Serial.println(" px");
  Serial.println("============================================");
}
...
void loop() {
  ...
  // Ball detection during WL_ARC is handled inside doWallLost() so
  // that the arc-resume path is taken on collection completion.
  if (robotState == STATE_WALL_FOLLOW && ballVisible()) {
    hardStop();
    ballPhase            = BP_TRACK;
    ballPhaseMs          = millis();
    arcInterruptedByBall = false;   // this is a normal wall-follow trigger
    robotState           = STATE_BALL_OVERRIDE;
